...
(() => {
  ...
  const savedAmwayCardOptions = $('#saved-amway-card-options2');

  const addCardContent = $('.mz-saved-credit-debit__add-card-content');

  const creditDebitButton = $('#credit-debit2');
  ...
  function handleCreditDebitUpdate(e) {
    ...
    if (
      // backward compatibility for full payment which doesn't have attr data-installment
