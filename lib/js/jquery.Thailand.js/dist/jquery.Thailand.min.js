 ...
 **/
$.Thailand=function(options){'use strict';options=$.extend({},$.Thailand.defaults,options);var preprocess=function(data){var lookup=[],words=[],expanded=[],t;if(data.lookup&&data.words){lookup=data.lookup.split('|');words=data.words.split('|');data=data.data}
...
provinces[i].map(function(amphoes){amphoes[i].map(function(districts){districts[i]=districts[i]instanceof Array?districts[i]:[districts[i]];districts[i].map(function(zipcode){var entry={district:t(districts[0]),amphoe:t(amphoes[0]),province:t(provinces[0]),zipcode:zipcode};if(i===2){entry.district_code=districts[1]||!1;entry.amphoe_code=amphoes[1]||!1;entry.province_code=provinces[1]||!1}
...
sum=max;if(sum){if(pos1&&pos2){sum+=similar_text(first.substr(0,pos2),second.substr(0,pos2),!1)}
if((pos1+max<firstLength)&&(pos2+max<secondLength)){sum+=similar_text(first.substr(pos1+max,firstLength-pos1-max),second.substr(pos2+max,secondLength-pos2-max),!1)}}
if(percentage===!1){return sum}else{if(first===second){return 100}else{if(firstLength>secondLength){return Math.floor(sum/firstLength*100)}else{return Math.floor(sum/secondLength*100)}}}},loadDB=function(callback){var type=options.database_type.toLowerCase(),xhr;if(type!=='json'&&type!=='zip'){type=options.database.split('.').pop()}
switch(type){case 'json':$.getJSON(options.database,function(json){callback(new JQL(preprocess(json)))}).fail(function(err){throw new Error('File "'+options.database+'" is not exists.')});break;case 'zip':if(!options.zip_worker_path){$('script').each(function(){var fragments=this.src.split('/'),filename=fragments.pop();if(filename==='zip.js'){zip.workerScriptsPath=fragments.join('/')+'/'}})}
xhr=new XMLHttpRequest();xhr.responseType='blob';xhr.onreadystatechange=function(){if(xhr.readyState===4){if(xhr.status===200){zip.createReader(new zip.BlobReader(xhr.response),function(zipReader){zipReader.getEntries(function(r){r[0].getData(new zip.BlobWriter(),function(blob){var
reader=new FileReader();reader.onload=function(){callback(new JQL(preprocess(JSON.parse(reader.result))))};reader.readAsText(blob)})})})}else{throw new Error('File "'+options.database+'" is not exists.')}}};xhr.open('GET',options.database);xhr.send();break;default:throw new Error('Unknown database type: "'+options.database_type+'". Please define database_type explicitly (json or zip)')}};loadDB(function(DB){$.Thailand.DB=DB;var i,key,defaultTemplates={empty:' ',suggestion:function(data){if(data.zipcode){data.zipcode=' » '+data.zipcode}
